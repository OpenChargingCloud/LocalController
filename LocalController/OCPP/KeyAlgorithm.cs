...
namespace cloud.charging.open.LocalController.OCPP
{
    ...
    /// <summary>
    /// A kind of key this local controller will generate for itself, and what
    /// it signs a certificate request with.
    /// </summary>
    /// <remarks>
    ...
    /// a charging station during a TLS handshake is something else entirely,
    /// and it is not decided here or by a list written from memory - it is
    /// found out by trying, on the machine this is running on. See
    /// <see cref="CanBePresented"/>.
    /// </remarks>
    /// <param name="Id">How it is written in the configuration and the API.</param>
                                      ...
                                      String  Remark)
    {
        ...
        /// <summary>
        /// What this local controller generates, in the order somebody should
        /// consider them: what every charging station handles, then what a
        /// stricter operator may ask for, then what is being prepared for.
        /// </summary>
        public static readonly IReadOnlyList<KeyAlgorithm> All = [

            new ("ecdsa-p256",         "ECDSA P-256 (secp256r1)",   "SHA256withECDSA",
                 "What a charging station with a small processor handles best, and what every one of them understands."),
            ...
            new ("rsa-4096",           "RSA 4096",                  "SHA256withRSA",
                 "The same, larger and slower; a charging station notices the difference at every handshake."),
        ...
        public AsymmetricCipherKeyPair Generate()
            ...
            => Id switch {
                   ...
                   _                    => throw new NotSupportedException($"'{Id}' is not a key this local controller generates!")
        ...
        /// <summary>
        /// Whether this machine can hold a certificate of this kind up to a
        /// charging station during a TLS handshake.
        /// </summary>
        /// <remarks>
        ...
        /// it is whether a charging station would get in.
        /// </remarks>
        ...
        private static async Task<Boolean> Handshake(X509Certificate2 Certificate)
        {
            ...
            // A deadline, because this runs while a controller is starting: a
            // TLS stack that neither completes nor refuses would otherwise hold
            // up a car park over a question about an algorithm.
        ...
        /// <summary>
        /// This algorithm, as the web interface reads it.
        /// </summary>
